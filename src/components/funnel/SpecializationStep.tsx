    ...
    <div className="max-w-2xl mx-auto">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-serif font-bold mb-4">
          {t("specialization.title")} <span style={{ color: "#e76830" }}>{t("specialization.titleAccent")}</span>
        </h2>
        <p className="text-muted-foreground text-lg">{t("specialization.subtitle")}</p>
      </div>
      <div className="space-y-6">
        <div className="space-y-2">
          <Label htmlFor="country-select" className="text-base font-medium">{t("specialization.countryLabel")}</Label>
          <Popover open={countryOpen} onOpenChange={setCountryOpen}>
            <PopoverTrigger asChild>
              <Button id="country-select" variant="outline" role="combobox" aria-expanded={countryOpen} className="w-full h-14 justify-between text-left font-normal">
                {selectedCountry ? selectedCountry.label : t("specialization.countryPlaceholder")}
                <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-full p-0" align="start">
              <Command>
                <CommandInput placeholder={t("specialization.searchCountries")} className="h-9" />
                <CommandList>
                  <CommandEmpty>{t("specialization.noCountryFound")}</CommandEmpty>
                  <CommandGroup>
                    {countries.map((co) => (
                      <CommandItem key={co.value} value={co.value} onSelect={(v) => { onUpdate({ country: v as CountryCode }); setCountryOpen(false); }}>
                        {co.label}
                        <Check className={cn("ml-auto h-4 w-4", country === co.value ? "opacity-100" : "opacity-0")} />
                      </CommandItem>
                    ))}
                  </CommandGroup>
                </CommandList>
          ...
          <p className="text-sm text-muted-foreground">{t("specialization.countryNote")}</p>
        </div>
        {role === 'yony_flowers_tutor' && (
          <motion.div className="p-6 rounded-lg border bg-card" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <h3 className="text-lg font-serif font-semibold mb-4 text-primary">{t("specialization.tutorTitle")}</h3>
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">{t("specialization.tutorDesc")}</p>
              <div className="flex items-start space-x-3">
                <Checkbox id="tutor-mission" checked={tutorMissionAccepted || false} onCheckedChange={(checked) => onUpdate({ tutorMissionAccepted: checked as boolean })} className="mt-1" />
                <div className="space-y-1">
                  <Label htmlFor="tutor-mission" className="text-sm font-medium cursor-pointer">{t("specialization.tutorAccept")}</Label>
                  <p className="text-xs text-muted-foreground">{t("specialization.tutorAcceptNote")}</p>
                </div>
              </div>
            </div>
          </motion.div>
        ...
        {country && (role !== 'yony_flowers_tutor' || tutorMissionAccepted) && (
          <motion.div className="flex items-center gap-3 p-4 rounded-lg bg-green-50 border border-green-200" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <CheckCircle className="w-5 h-5 text-green-600" />
            <span className="text-sm font-medium text-green-700">{t("specialization.confirmed", { country: selectedCountry?.label })}</span>
          </motion.div>
        )}
      </div>
    </div>
